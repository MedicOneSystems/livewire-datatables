                    ...
                    <div class="flex rounded-lg w-96 shadow-sm">
                        <div class="relative flex-grow focus-within:z-10">
                            ...
                            <input wire:model.lazy="search"
                                class="block w-full py-3 pl-10 text-sm border-gray-300 leading-4 rounded-md shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 focus:outline-none placeholder:text-ellipsis"
                ...
                @if (count($this->massActionsOptions))
                    <div class="flex items-center justify-center space-x-1">
                        ...
                        <select wire:model="massActionOption"
                            class="px-3 text-xs font-medium tracking-wider uppercase bg-white border border-green-400 space-x-2 rounded-md leading-4 focus:outline-none"
                        ...
                        <button wire:click="massActionOptionHandler"
                            class="flex items-center px-4 py-2 text-xs font-medium tracking-wider text-green-500 uppercase bg-white border border-green-400 rounded-md leading-4 hover:bg-green-200 focus:outline-none"
                            ...
                            @foreach ($this->columns as $index => $column)
                                ...
                                @elseif($column['type'] === 'checkbox')
                                    @unless ($column['hidden'])
                                        <div
                                            class="flex justify-center table-cell w-32 h-12 px-6 py-4 overflow-hidden text-sm font-medium tracking-wider text-left text-gray-600 uppercase align-top border-b border-gray-200 bg-gray-50 leading-4 focus:outline-none">
                                            <div
                        ...
                        <div class="table-row text-gray-800 font-medium {{ $loop->even ? 'bg-gray-50' : 'bg-white' }}">
                            @foreach ($this->columns as $column)
                                @if ($column['hidden'])
                                    @if ($hideable === 'inline')
                                        <div
                                            class="table-cell px-6 py-4 whitespace-nowrap truncate w-5 overflow-hidden align-top">
                                        </div>
                                    @endif
                                @elseif($column['type'] === 'checkbox')
                                ...
                                @else
                                    <div
                                        class="table-cell px-6 py-4 whitespace-nowrap truncate @if ($column['contentAlign'] === 'right') text-right @elseif($column['contentAlign'] === 'center') text-center @else text-left @endif {{ $this->cellClasses($row, $column) }}">
                                        {!! $row->{$column['name']} !!}
                                    </div>
                                @endif
                            @endforeach
