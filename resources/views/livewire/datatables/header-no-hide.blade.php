@unless($column['hidden'])
    <div@if (isset($column['tooltip']['text'])) title="{{ $column['tooltip']['text'] }}" @endif class="relative table-cell h-12 overflow-hidden align-top"
                    ...
                    @elseif($this->multisort === true)
                        @foreach($sort as $key => $value)
                            @if($this->getIndexFromValue($value) == $index && ($direction = $this->getColumnDirection($value)))
                                ...
                                @if($direction === 'desc')
                                    <x-icons.chevron-down wire:loading.remove
                                                          class="w-6 h-6  text-green-600 stroke-current"/>
                                    <div>{{$key + 1}}</div>
                                @endif
                            @endif
                        @endforeach
