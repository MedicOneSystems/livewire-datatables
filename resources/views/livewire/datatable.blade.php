<div class="">
    ...
    <div class="mb-4 grid grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-2">
        ...
        <button wire:click.prefetch="toggle('{{ $index }}')" class="px-3 py-2 rounded text-white text-xs focus:outline-none {{ $column['hidden'] ? 'bg-blue-100 hover:bg-blue-300 text-blue-600' : 'bg-blue-500 hover:bg-blue-800' }}">
    ...
    <div class="mt-4 p-4 rounded overflow-hidden align-middle min-w-full shadow sm:rounded-lg border-b border-gray-200 bg-white">
        <div class="h-6 flex justify-between items-center">
            ...
            <button wire:click="clearTimeFilter" class="@if(!isset($times['column']) || $times['column'] === '') hidden @endif px-2 py-1 flex items-center uppercase tracking-wide border-2 border-transparent hover:bg-red-400 text-gray-600 hover:text-white rounded-full focus:outline-none text-xs space-x-1">
                <span>CLEAR</span>
                <x-icons.x-circle />
            </button>
        </div>
    ...
    <div class="mt-4 p-4 rounded overflow-hidden align-middle min-w-full shadow sm:rounded-lg border-b border-gray-200 bg-white">
        ...
        <div class="mt-2 grid grid-cols-3 gap-4">
            ...
            <div class="w-full relative">
                ...
                <div class="relative">
                    <select name="{{ $filter['label'] }}" class="w-full form-select" wire:input="doBooleanFilter('{{ $i }}', $event.target.value)">
                        <option value=""></option>
            ...
            <div class="w-full relative">
                ...
                <div class="relative">
                    <input name="{{ $filter['label'] }}" type="text" class="w-full form-input" wire:input.lazy="doTextFilter('{{ $i }}', $event.target.value)" />
                </div>
            </div>
            @endforeach
